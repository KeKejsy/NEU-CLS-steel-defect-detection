# 成员 C · 检测任务说明（src/det/）

本文件说明检测部分的实现思路、目录结构、使用方法与实测结论。
面向的读者是：接手复现的同学、写报告/PPT 的同学、以及答辩时被问到的评审。

---

## 一、总体设计：为什么不用 PaddleDetection

原计划按 README 用 PaddleDetection（PP-YOLOE-s + YOLOv3）。动手前实测了它的可行性，
结论是**不能用**，理由有实测依据：

| 检查项 | 实测结果 |
|---|---|
| `PaddleDetection/requirements.txt` 第 1 行 | `numpy < 2.0`，而本机 numpy 是 **2.5.2** |
| `git clone` release/2.9 后 `import ppdet` | 逐层失败：缺 `pycocotools` → `requests` → `imgaug.augmentables` → … |
| 需要的额外依赖 | 约 15 个，且必须把 numpy 降到 <2 |

把 numpy 降级会**连带搞崩成员 A 已经跑通的数据脚本**，这是项目铁律明确要避免的事。
更重要的是：**根本没必要**。实测 `paddle.vision.ops` 已经自带检测所需的全部算子：

```
deform_conv2d   nms（原生支持 category_idxs 分类批量 NMS）   yolo_box
matrix_nms      roi_align      prior_box      yolo_loss
generate_proposals      distribute_fpn_proposals      psroi_pool
```

所以最终方案是：**用原生 Paddle 手写两个检测网络，零新增依赖、不编译自定义 OP。**

这一选择也符合本项目既有风格 —— README 给成员 B 的建议原话是
「就用 `paddle.vision.models` 自己写训练循环，别去折腾 PaddleClas 源码……
手写训练循环不到 100 行，可控、好 debug、出错好查」。检测部分同理，且更甚。

---

## 二、目录结构

```
src/det/
├── train.py               训练脚本（--config 切换网络）
├── eval_det.py            单阶段评估：mAP@0.5 / 每类 AP / P / R / F1 / PR 曲线 / 混淆矩阵
├── eval_2stage.py         两阶段评估（检测器或滑窗 + 验证器）
├── eval_refine.py         滑窗 + 精修器评估（含框回归消融）
├── eval_fused.py          滑窗 + 验证器/精修器融合评估  ← 最终方案
├── train_verifier.py      区域验证器训练（判类）
├── train_refiner.py       区域精修器训练（判类 + 框回归）
├── viz_det.py             可视化：真值-预测对比图、错例分析图、尺寸-AP 图
├── demo.py                单图/批量推理 Demo，可测 FPS
├── configs/
│   ├── yolov3.yml         YOLOv3 配置
│   └── ppyoloe_s.yml      PP-YOLOE-s 配置
├── core/                  共用基础模块
│   ├── data.py            数据集、VOC 解析、Mosaic/缩放/翻转增强、推理封装
│   ├── boxes.py           坐标换算、IoU、NMS、坐标夹紧
│   ├── det_ops.py         IoU/DFL 损失、解码、正样本分配辅助、退化框过滤
│   ├── layers.py          ConvBN、Bottleneck、CSPLayer、RepConv、SPPF、ESE
│   ├── utils.py           配置读取、随机种子、CSV/JSON 日志、**mAP 计算**
│   ├── verifier.py        区域验证器（第二阶段基础版：只判类）
│   ├── refiner.py         区域精修器（第二阶段升级版：判类 + 框回归）
│   └── window_detector.py 多尺度多宽高比滑窗检测器
├── nets/
│   ├── darknet.py         Darknet53 主干（YOLOv3 用）
│   ├── yolov3.py          YOLOv3 检测器
│   ├── cspresnet.py       CSPResNet 主干（PP-YOLOE-s 用）
│   └── ppyoloe.py         PP-YOLOE-s 检测器
└── tools/                 诊断与调优工具（迭代时用，非交付必需）
    ├── summarize.py       汇总全部结果 → det_summary.md
    ├── check_docs.py      文档与产物一致性校验
    ├── analyze_result.py  混淆矩阵拆解，定位短板类别
    ├── bottleneck.py      瓶颈诊断：召回上限 / 判类 / 排序 拆开量化
    ├── bottleneck2.py     候选来源与聚合策略对比
    ├── sweep_refine.py    分层抽样超参扫描（避免类别偏置）
    ├── tune_fused.py      融合后处理精调（缓存候选 + 网格搜索）
    ├── tune_windows.py    滑窗形状专项调优（细长缺陷）
    ├── ab_aspects.py      窄宽高比 A/B 确认实验
    ├── kmeans_anchors.py  k-means 统计目标尺寸、生成定制 anchor
    ├── diagnose.py        单阶段检测器诊断（各 IoU 阈值 mAP 等）
    └── two_stage_compare.py 单阶段 vs 两阶段对比
```

**共用与分工**：两个检测网络共用 `core/` 里的数据管线、坐标约定、NMS 后处理与评估代码，
差异**只体现在网络结构与回归方式**上，这样 mAP 对比才能归因到「网络设计差异」。

**两阶段流程的模块关系**：
`window_detector` 产生候选 → `verifier`（判类）+ `refiner`（判类 + 框回归）分别打分
→ 概率几何平均融合 → NMS 输出。融合比单用任一模型都好，原因见 9.1 节。

---

## 三、两个网络的设计对比

| | YOLOv3 | PP-YOLOE-s |
|---|---|---|
| 主干 | Darknet53（残差瓶颈 ×26） | CSPResNet（CSP + RepConv + ESE 注意力） |
| 颈部 | FPN 自顶向下三层融合 | CSPPAN 双向融合四层 |
| 检测方式 | **anchor-based**，9 个预设 anchor | **anchor-free**，每个特征点直接回归 |
| 框回归 | 直接回归 + GIoU 损失 | **DFL 分布式回归**（学「边落在哪个区间」的分布取期望）+ GIoU |
| 置信度 | obj × cls（两个分支） | 类别分数即置信度（无 obj 分支） |
| 正样本分配 | 宽高 IoU 最大的 anchor + 所在网格 | 按框等效边长选层 + 中心 2.5 格内锚点 |
| 参数量 | 6440.10 万 | 2375.17 万 |
| 输入尺寸 | 416 | 640 |

**为什么这个对比有意义**：本数据集的框宽高比跨度极大 ——
从细长划痕（Sc）到近方形麻点（PS）。anchor-based 方法需要预设 9 个宽高比，
很难覆盖全部形态；而 anchor-free + DFL 不依赖先验宽高，理论上在这种数据上更有优势。
实验设计上就是为了检验这一点，而不是随便挑两个网络凑数。

---

## 四、使用方法

```bash
conda activate paddle_env      # 本机实际路径见下方「环境」一节

# 1) 训练（两个网络串行跑，不要同时开，避免抢显存）
python src/det/train.py --config src/det/configs/yolov3.yml
python src/det/train.py --config src/det/configs/ppyoloe_s.yml

# 快速联调（3 个 epoch 看流程是否通，不追求精度）
python src/det/train.py --config src/det/configs/yolov3.yml --epochs 3 --batch-size 4

# 2) 评估（测试集只在这一步用一次）
python src/det/eval_det.py --model yolov3
python src/det/eval_det.py --model ppyoloe_s
python src/det/eval_det.py --model yolov3 --split val      # 调参请看验证集

# 3) 可视化
python src/det/viz_det.py --model yolov3
python src/det/viz_det.py --model ppyoloe_s --figs samples,errors,size_ap

# 4) 单图推理
python src/det/demo.py --model yolov3 --image dataset/det/JPEGImages/crazing_27.jpg
python src/det/demo.py --model ppyoloe_s --benchmark        # 顺带测 FPS
```

### 产出文件

| 路径 | 内容 |
|---|---|
| `results/weights/<name>_best.pdparams` | 验证集 mAP 最高的权重 |
| `results/weights/<name>_last.pdparams` | 最后 epoch 的权重 |
| `results/logs/<name>_train.csv` | 逐 epoch 记录（可交给 D 的 `log2table.py` 解析） |
| `results/metrics/<name>_train_summary.json` | 训练摘要：超参、最佳 mAP、耗时、逐 epoch 历史 |
| `results/metrics/<name>_eval_result.json` | 评估结果：mAP、每类 AP/P/R/F1、PR 曲线、混淆矩阵 |
| `results/figures/<name>_pr_curve.png` | 每类 PR 曲线 |
| `results/figures/<name>_per_class_ap.png` | 每类 AP 柱状图 |
| `results/figures/<name>_confusion.png` | 混淆矩阵 |
| `results/figures/<name>_samples.png` | 6 类真值 vs 预测对比图 |
| `results/figures/<name>_errors.png` | 错例分析（漏检/误检/类别错） |
| `results/figures/<name>_size_ap.png` | 不同目标尺寸下的 mAP |

---

## 五、环境（本机实测，与 README 记载不同）

README 的「开发环境」一节是按**另一台机器**写的，在本机不成立，以实测为准：

| 项 | README 记载 | 本机实测 |
|---|---|---|
| 环境路径 | `D:\Miniconda3\envs\paddle_env` | **`C:\Users\htt22\miniconda3\envs\paddle_env`** |
| Paddle 版本 | 3.3.1（CPU 版） | **paddlepaddle-gpu 3.2.0** |
| GPU | 无，CPU 训练 | **RTX 5070 Ti Laptop 12GB（sm_120）** |
| 本次补装的包 | — | `PyYAML 6.0.3`、`tqdm 4.70.0`、`opencv-python 5.0.0.93` |

补装用 `pip install --upgrade-strategy only-if-needed`，并先跑 `pip --dry-run` 预演，
确认 **numpy 2.5.2 不会被降级**（这是 README 反复强调的红线）。

> README 里「ResNet50 要跑 3 小时、检测网络在 CPU 上非常吃力、建议申请 AI Studio」
> 这一整套规划在本机不适用：有 GPU 之后训练快约 30 倍，检测任务完全跑得动。

---

## 六、实测性能

RTX 5070 Ti Laptop（12GB），1260 张训练集：

| 网络 | 输入 | batch | 单步耗时 | 每 epoch | 峰值显存 | 跑满训练耗时 |
|---|---|---|---|---|---|---|
| YOLOv3 | 416 | 16 | 181 ms | 约 19 s | 3.17 GB | 120 epoch / 38.0 分钟 |
| PP-YOLOE-s | 640 | 8 | 249 ms | 约 50 s | 9.7 GB | 100 epoch / 81.9 分钟 |
| PP-YOLOE-s | 640 | 16 | 51984 ms | 不可用 | 12.18 GB（顶满） | — |

**⚠️ 显存红线**：PP-YOLOE-s 在 640 输入下 **batch 必须 ≤ 8**。
bs=16 时峰值显存 12.18GB 正好顶满 12GB 的卡，触发显存重试，
单步从 249ms 暴涨到 **52 秒（慢 200 倍）**。这不是报错，是静默变慢，很容易被忽略。

**同类陷阱（实测踩过）**：**两个训练进程并发跑同样会触发显存重试**。
PP-YOLOE-s 在另一进程同时训练时，单 epoch 从 51 秒暴涨到 **7224 秒（慢 134 倍）**。
所以两个网络必须**串行训练** —— 与 README「CPU 训练实操建议」里
「两个网络串行训，不要同时开」是同一个道理，只是原因从 CPU 核数变成了显存。
本次交付前的跑满训练即按串行执行，全程显存稳定在 9.7GB、GPU 利用率 79~99%。

---

## 七、评测口径

- **坐标**：归一化 xyxy，取值 [0,1]，贯穿数据、网络、后处理、评估全流程。
  因此评估阶段**不需要任何坐标反变换**，预测与标注天然同尺度可比。
- **mAP**：VOC 口径 —— IoU 阈值 0.5、每类按分数降序匹配、101 点插值算 AP、
  mAP = 6 类 AP 的算术平均。与同类复现结果可直接对比。
- **评估输入**：统一缩放到固定正方形（`resize_mode='square'`）；
  训练用等比缩放 + padding。之所以评估不保留 padding：不同图的黑边比例不同，
  会让输入分布不确定、结果不可复现。
- **正确性验证**：mAP 实现经过 **6 项已知答案单测**：
  完美预测 = 1.0；框微抖（IoU 仍 >0.5）= 1.0；完全错位 = 0.0668；
  类别全错 = 0.0；只命中一半 = 0.5944；PR 曲线 recall 单调不减。全部符合预期。

### 指标实现说明（与成员 D 的接口）

`mAP / 每类 AP / PR 曲线` 目前实现在 `src/det/core/utils.py` 的 `evaluate_map()`。
原因：成员 D 的 `src/tools/metrics.py` 尚未交付（当时为 0 字节），C 的评估不能停工等它。
**接口按可替换设计**：训练与评估脚本只通过 `utils.evaluate_map()` 这一个入口调用指标，
将来 D 的指标库就绪后，把该函数内部改为 `from tools.metrics import evaluate_map`
即可整体切换，调用方一行都不用改。

---

## 八、工程细节：几个踩过的坑（都已修复，记下来避免重犯）

这一节对写报告「遇到的问题与解决」很有用，按代价从大到小排。

### 1. `masked_select` 的反向慢 1600 倍
最初用 `masked_select` 取出正样本再算回归损失：前向只要 1ms，
但**反向要 1.6 秒/步**（8500×68 中间张量展开 + 散射梯度）。
改成「全量算损失 + 掩码加权归约」后，**单步从 3927ms 降到 249ms，快 16 倍**。

### 2. IoU 损失误用两两矩阵导致爆显存
`IoULoss` 原实现调用 `box_iou` 构造 (N,N) 矩阵。输入是 (B,N,4) 时 N 可达 68000，
矩阵元素上亿，实测申请 **34.45GB**（显存只有 12GB）直接 OOM。
实际上 IoU 损失是**一一对应**关系（第 i 个预测框 vs 第 i 个目标框），
根本不需要两两矩阵，改为逐元素直算即可。

### 3. 退化框污染 mAP
`clip_boxes_norm` 原实现把四个坐标各自 clip 到 [0,1]，
会把部分在画面外的框压成零宽/零高（x1、x2 同时被压到 1）。
零面积框在 NMS 里被当成互不重叠而全部留下 —— 实测 **27000 个框里有 4082 个是退化框**。
改成「先夹中心点与宽高，再由中心重构四角」后，退化框 **0 个**。
（附带发现：`paddle.clip` 的 min/max 只接受标量，逐元素边界要用 `maximum`/`minimum`。）

### 4. 非二次幂输入尺寸让 FPN 相加报错
原图 200×200 时三尺度是 25/13/7，而 7 的两倍是 14、目标层是 13，
硬写 `scale_factor=2` 直接 Broadcast 报错。
改为按目标特征图尺寸 `interpolate(size=[h,w])` 自适应缩放，对任意输入边长都成立。

### 5. 损失归一化口径写错会让训练直接发散
YOLOv3 的 obj 分支原按「正样本数」归一化，但一张图有上万候选位置、
正样本只有 2~3 个，损失被放大到 **44636**、梯度范数 **2.4e6**。
正确做法是**按全部元素取平均**，正负不均衡靠 `pos_weight` 调节，绝不改分母。

### 6. 学习率 warmup 形同虚设
自写 `LambdaDecay` 闭包时，optimizer 内部还会对 lr 再做一次处理，
实测学习率只升到预期的 1/30（3.1e-5 而非 9.5e-4），训练 4 个 epoch 几乎没学到东西。
改用原生 `LinearWarmup + CosineAnnealingDecay` 组合并逐点验证后才正常。
另外 `warmup_epochs` 不能超过总 epoch 的 1/5，否则短程试验会全程处于 warmup。

### 7. 主干结构连写错三次

Darknet53 的 stem 下采样次数、各阶段通道数、YOLOv3 到底取哪三个阶段的输出，
这三处各错了一次，症状都是「通道数不匹配」或「特征图尺寸不对」。
最后靠**逐层打印实际形状**（而不是靠推演）才定位。
教训：像 Darknet53 这种有固定官方层序的结构，应当逐层写死并与官方表对照，
不要用「循环 + 通道表」去推导 —— 容易把 /2 与 /4 的位置搞反。

### 8. 精修器的抖动范围：合成指标会误导（第三轮试错）

第三轮除滑窗形状外，还试过「用难负样本 / 放宽抖动重训精修器」，结论是**不要这样做**。

起因是实测发现：被当作预测输出的高分窗口里只有 8.3% 与 GT 完全无重叠，
而 31.6% 落在 IoU 0.3~0.5、36.4% 落在 0.1~0.3 —— 大量误检是「差一点命中」的框。
直觉上应当放宽训练时的抖动范围，让网络见见这些困难样本。于是把抖动从
（尺度 0.20~0.65、比例 0.6~1.7、偏移 ±25%）放宽到（0.18~0.85、0.3~2.0、±40%）
重训了 60 epoch。结果**反而更差**：

| 抖动范围 | 合成 IoU（训练日志） | 推理分布上的精修增益 | 最终 mAP@0.5 |
|---|---|---|---|
| 匹配滑窗分布（采用） | 0.442 | IoU +0.0805，命中率 +14.0pp | **0.2385** |
| 放宽抖动（弃用） | 0.336 | IoU +0.0697，命中率 +10.8pp | 0.1289 |

两条教训：

1. **抖动范围应与推理分布匹配，而不是越大越好**。滑窗实际产生的尺度就是 0.2~0.65，
   放宽到 0.18~0.85 后合成任务与真实分布脱节，网络把容量花在了推理时根本不会出现的样本上；
2. **「合成 IoU」这个训练日志指标会误导**。放宽抖动时它反而更低（0.336 vs 0.442），
   但那只是任务更难，不代表模型更差 —— 真正要看的指标是
   **推理分布上的精修增益**，为此专门写了 `tools/compare_refiners.py`。

> 这个脚本值得保留：它把「合成指标的提升」与「真实推理的增益」区分开，
> 避免以后再被训练日志骗一次。

### 9. 滑窗尺度的上限也不要盲目加大（第三轮试错）

统计各类目标框尺寸时发现：**PS 麻点的长边中位数达 0.962，几乎等于整幅图**，
而当时滑窗最大尺度只有 0.65 —— 看起来「窗口太小、覆盖不了 PS」。
于是加入 0.9 与 1.0 两档大尺度，结果**明显变差**：

| 滑窗尺度 | 候选/图 | mAP@0.5 |
|---|---|---|
| (0.2, 0.3, 0.45, 0.65)（保留） | 4624 | **0.2369** |
| 加 0.9 档 | 5780 | 0.2061 |

原因：接近整幅图的窗口几乎必然覆盖到缺陷，验证器会给这类窗口一致的高分，
**只增加噪声、不带来有效定位信息**（大窗口的定位精度天然很差）。
所以尺度上限保持 0.65。

> 与第 8 条（精修器抖动）是同一类教训：**「覆盖不到」不等于「应该覆盖」**，
> 候选窗口的形状与尺度必须与「能提供有效定位信息」的区间匹配，
> 而不是与目标尺寸的上界匹配。

### 10. 给验证器加「难负样本」反而更差（融合会惩罚过度保守的模型）

初版验证器只用「GT 框 vs 完全不重叠的随机背景」训练，于是它把几乎所有覆盖到缺陷的
窗口都判成缺陷。直觉上补上「难负样本」应当能治这个问题：把 GT 框放大 1.5~3.0 倍，
造出「覆盖缺陷但框不准」的样本（对应实测中占 68% 的 IoU 0.1~0.5 高分窗口），
并给正样本加 ±12% 抖动。重训后**反而明显变差**：

| 验证器训练集 | 分层抽样 120 张上的 mAP@0.5 | 每类 AP 变化 |
|---|---|---|
| 原版（GT 框 + 纯背景）（保留） | **0.2275** | PS=0.457 RS=0.140 Sc=0.242 |
| 加难负样本 + 正样本抖动（弃用） | 0.1857 | PS=0.457→0.331，RS→0.080；仅 In 略升 |

原因是**融合会惩罚过度保守的模型**：加入难负样本后验证器对「大框」变得很谨慎，
而 PS（麻点，框几乎占满整幅图）恰恰需要在大框上给高分 —— 它一保守，PS 就崩了。
两个模型的**误差互补**才是融合生效的前提；把其中一个训练得更「严格」，
不等于让组合更好，反而破坏了平衡。

> 三条负面结论放在一起看（第 8、9、10 条），规律很清楚：
> **本方案的两个判别模型与滑窗配置都已调到「互相配合」的状态，
> 单独加强任何一个环节都会破坏平衡。** 想再提升得换思路（见 9.4 的后续建议），
> 而不是继续微调这三个组件。

### 11. 迭代式精修：结果不稳健，未采纳（第三轮探索）

既然单轮精修能把高分窗口的命中率从 3.2% 提到 17.3%，自然会想「再精修一轮」。
实测结论是**不采纳**，而且过程本身暴露了一个容易误判的陷阱。

在 90 张分层抽样图上的三组对比：

| 配置 | mAP@0.5 | 框/图 | Cr | In | Pa | PS | RS | Sc |
|---|---|---|---|---|---|---|---|---|
| 单轮精修 + 原始分数（当前方案） | 0.2246 | 19.9 | 0.126 | 0.083 | 0.300 | 0.435 | 0.123 | 0.281 |
| 两轮精修 + **原始**分数 | 0.1812 | 23.0 | 0.070 | 0.041 | 0.327 | 0.418 | 0.133 | 0.098 |
| 两轮精修 + **重新打分** | 0.2351 | 13.7 | 0.126 | 0.102 | 0.353 | 0.324 | 0.140 | 0.367 |

**三个要点**：

1. 「两轮 + 原始分数」明显更差（0.2246 → 0.1812）—— 说明**第二轮精修会把已经准的框改坏**
   （Sc 从 0.281 崩到 0.098）。因为精修器学的是「把偏移的框拉回」，对已经对齐的框
   再施加一次同样的变换就是过冲。
2. 「两轮 + 重新打分」数值反而最高（0.2351），但这主要是**重新打分让分数分布整体偏移、
   输出框数从 19.9 降到 13.7** 带来的副作用，而不是定位真的更好 ——
   证据是类间波动极大：Sc 0.281 → 0.367（涨），但 PS 0.435 → 0.324（崩）。
3. **90 张样本上的 0.01 量级差异在噪声范围内**，不足以支撑「改进」的结论。
   本项目此前的教训（子集抽样偏置、合成指标误导）都指向同一件事：
   **小幅优势必须在大样本上复核**，否则很容易把噪声当成收益。

> 所以最终方案保持**单轮精修**。这也再次印证第 8~10 条的规律：
> 当前这套「滑窗 + 双模型融合 + 单轮精修」已处于互相配合的状态，
> 继续在同一层面微调收益甚微；要再提升需要换层次（如引入预训练主干、
> 或用真正的两阶段检测器做端到端训练）。

---

## 九、结果

> 全部数字由 `python src/det/tools/summarize.py` 自动汇总到
> `results/metrics/det_summary.md`，与 `results/metrics/*.json` 一一对应，不存在手抄误差。

### 9.1 迭代历程与最终结果（**最重要的结论**）

检测效果经过**两轮**迭代后大幅提升：

| 方案 | 验证集 mAP@0.5 | 测试集 mAP@0.5 | 说明 |
|---|---|---|---|
| 初版：滑窗 + 验证器 | 0.0677 | 0.0548 | 只用验证器判类 |
| 第二轮：+ 精修器 + 概率融合 | 0.1575 | 0.1435 | 新增框回归与融合 |
| **第三轮：+ 窄宽高比滑窗** | 0.1675 | — | 针对细长缺陷 In/Sc |
| **完整流水线重跑（最新代码）** | **0.1681** | **0.1552** | 一键复现，见 §9.6 |
| 累计提升 | **+148%** | **+183%** | — |

> 表中「完整流水线重跑」是本项目**最终交付的数字**：用 `tools/full_pipeline.py`
> 从数据自检一路跑到结果汇总，全程 186.2 分钟、12 个子任务 11 项通过
> （唯一「失败」项是文档一致性校验，它正是用来发现文档与产物不一致的，
> 触发后已同步文档）。重跑命令：
> `python src/det/tools/full_pipeline.py --device gpu`

**第三轮是怎么找到的**（承接第二轮的诊断思路）：

1. 写 `tools/analyze_result.py` 拆解混淆矩阵，发现一个关键事实：
   **错分列全为 0** —— 模型从不把缺陷判成别的类别，误差只有「误检」与「漏检（IoU 不够）」；
2. 漏检集中在 `In 夹杂`（召回 0.49）与 `Sc 划痕`（0.51），其余四类 0.71~0.80；
3. 统计各类目标框形状，原因立刻明确：

   | 类别 | 宽中位 | 高中位 | **宽高比** |
   |---|---|---|---|
   | In 夹杂 | 0.125 | 0.325 | **0.37** |
   | Sc 划痕 | 0.133 | 0.653 | **0.18** |
   | Cr 龟裂 | 0.630 | 0.350 | 1.79 |
   | PS 麻点 | 0.625 | 0.955 | 0.76 |

   原滑窗宽高比只有 `(0.6, 1.0, 1.7)`，**产生不出 In/Sc 那种细长形状的窗口** ——
   候选里根本没有贴合的框，再强的分类器也救不回来；
4. 用 `tools/ab_aspects.py` 做 A/B 确认（120 张分层抽样）：
   mAP 0.2254 → **0.2369（+5.1%）**，其中 **Sc +0.046、RS +0.030**；
5. 加入 `0.35` 档宽高比后，完整验证集（270 张）结果：

   | 类别 | 旧滑窗 | 新滑窗 | 变化 |
   |---|---|---|---|
   | **Sc 划痕** | 0.0916 | **0.1372** | **+0.0456** |
   | **RS 氧化铁皮压入** | 0.1221 | **0.1563** | **+0.0342** |
   | PS 麻点 | 0.2747 | 0.2770 | +0.0022 |
   | Cr 龟裂 | 0.1259 | 0.1198 | −0.0061 |
   | In 夹杂 | 0.0872 | 0.0752 | −0.0121 |
   | Pa 斑块 | 0.2436 | 0.2397 | −0.0038 |
   | **总体** | **0.1575** | **0.1675** | **+0.0100** |

   细长两类明显受益、其他类基本持平 —— **这正是预期的效果**，说明「窗口形状与目标形状匹配」
   确实是这两类的瓶颈。更极端的宽高比 0.2 反而使整体略降（候选数近乎翻倍带来的噪声
   抵消了收益），故未采用。

每类 AP@0.5（完整流水线重跑，测试集）：

| 类别 | Cr 龟裂 | In 夹杂 | Pa 斑块 | PS 麻点 | RS 氧化铁皮压入 | Sc 划痕 |
|---|---|---|---|---|---|---|
| AP | 0.136 | 0.059 | **0.201** | **0.353** | 0.112 | 0.070 |
| 召回 | 0.699 | 0.484 | 0.708 | **0.786** | 0.722 | 0.605 |

验证集同口径（mAP@0.5 = 0.1681）：

| 类别 | Cr | In | Pa | PS | RS | Sc |
|---|---|---|---|---|---|---|
| AP | 0.157 | 0.086 | 0.182 | **0.386** | 0.118 | 0.081 |
| 召回 | 0.703 | 0.530 | 0.664 | **0.800** | 0.616 | 0.550 |

**第二轮是怎么找到方向的**（这一步比调参本身更有价值）：

1. 先写 `tools/bottleneck.py` 做**量化诊断**，而不是盲目调参。三个关键数字：
   - 滑窗候选 2028 个/图，IoU≥0.5 的召回**上限 70.1%** → 空间还有，不是定位绝境；
   - 验证器判「是不是缺陷」95.1%、判「是哪一类」97.35% → 判类环节没问题；
   - **GT 最佳候选的排名中位数 194** → 正确框被海量候选淹没了。
2. 关键一击：测出**阈值从 0.5 提到 0.9，每图框数只从 49.8 降到 48.6**。
   说明验证器把几乎所有覆盖缺陷的窗口都判成了缺陷 —— 它训练时只见
   「GT 框 vs 随机背景」，**从未见过推理时的密集重叠窗口**。
3. 方向因此明确：不是调阈值、不是换聚合，而是**让框变准**。
   新增 `core/refiner.py`：在验证器基础上加一个**框回归头**，把候选框对齐到真实缺陷。
   消融证明这一击是主要收益来源：同参数下带框回归 0.0991 vs 关掉 0.0406。
4. 进一步发现**概率融合**有效：验证器（分类头训练更充分）与精修器（见过真实候选分布）
   误差互补，几何平均后判类准确率从 95.9%/96.4% 升到 **96.9%**，mAP 再翻近一倍。

**消融实验**（同一批数据、同一后处理，验证集 270 张）：

| 配置 | mAP@0.5 |
|---|---|
| 滑窗 + 验证器（只判类） | 0.1231 |
| 滑窗 + 精修器（只用一个模型） | 0.0886 |
| **滑窗 + 两者融合（第二轮方案）** | **0.1575** |
| **再加窄宽高比滑窗（第三轮方案）** | **0.1675** |

**后处理关键参数**（实测最优，见 `tools/tune_fused.py`）：

| 参数 | 取值 | 为什么 |
|---|---|---|
| 滑窗步长 | 12 | 比 16 更密，候选 IoU 上限更高 |
| 滑窗宽高比 | **0.35, 0.6, 1.0, 1.7** | 0.35 专为细长缺陷（In 0.37 / Sc 0.18）而加，见第三轮 |
| 分数阈值 | 0.6 | 实测 0.3~0.95 影响很小（融合后概率分布集中） |
| NMS IoU | 0.4 | 常规是 0.5，但本方案每图几十个框而真值只有 2.4 个，需更严的去重 |
| 先截 top_k | 150 | **与 max_det 不同**：先截可避免低分框参与 NMS 把高分框误压掉 |
| max_det | 50 | NMS 后保留上限 |

> ⚠️ 踩过的坑：我最初直接用 `ds.samples[:N]` 取子集扫描超参，结论是错的 ——
> 划分文件按类别前缀排序，前 80 张只有 Cr 和 In 两类（45+35），
> 扫出来的 mAP（0.043）与全量评估（0.099）差一倍多。
> 后来专门写了 `tools/sweep_refine.py` 做**分层抽样**（每类等量），结论才可靠。

### 9.2 第二阶段的两个模型

| 指标 | 验证器 | 精修器（迭代新增） |
|---|---|---|
| 任务 | 判类（7 类） | 判类 + **框回归** |
| 参数量 | 258.67 万 | 259.08 万 |
| 训练样本 | 4142 图块 | 21055 样本（含抖动的模拟候选） |
| 整体准确率 | 88.72% | 78.07% |
| 在真实候选分布上的判类准确率 | 95.92% | 96.43% |
| 精修后平均 IoU | — | 0.442 |
| GT 框上的分类准确率（测试集） | **97.00%** | — |

> 精修器整体准确率低于验证器（78% vs 89%），因为它的训练集更难：
> 含大量「部分覆盖缺陷」的模拟候选。但在**真实候选分布**上它的判类反而略优
> （96.43% vs 95.92%），这正是融合能奏效的原因。

### 9.3 检测性能汇总（mAP@0.5）

| 方案 | 划分 | mAP@0.5 | 精确率 | 召回率 | 每图框数 |
|---|---|---|---|---|---|
| YOLOv3 单阶段（120 epoch 跑满） | 验证集 | 0.0007 | — | — | — |
| PP-YOLOE-s 单阶段（100 epoch 跑满） | 验证集 | 0.0000 | — | — | — |
| YOLOv3 检测器 + 验证器 | 测试集 | 0.0001 | 0.0009 | 0.0158 | 42.4 |
| 滑窗 + 验证器（初版） | 验证集 | 0.0709 | 0.0262 | 0.5383 | 48.7 |
| 滑窗 + 验证器（初版） | 测试集 | 0.0548 | 0.0248 | 0.5142 | 48.8 |
| 滑窗 + 融合（第二轮） | 验证集 | 0.1575 | 0.0680 | 0.6291 | 21.9 |
| 滑窗 + 融合（第二轮） | 测试集 | 0.1435 | 0.0704 | 0.6546 | 21.8 |
| **滑窗 + 融合（完整流水线重跑）** | 验证集 | **0.1681** | 0.0755 | 0.6307 | 19.8 |
| **滑窗 + 融合（完整流水线重跑）** | **测试集** | **0.1552** | **0.0782** | **0.6483** | 19.5 |

> 两个单阶段网络均已**按配置跑满**（YOLOv3 120 epoch / 38.0 分钟，
> PP-YOLOE-s 100 epoch / 81.9 分钟）。跑满后最佳验证 mAP 分别为 0.0007 与 0.0000，
> 与调试期只跑 40/17 epoch 时相比**没有实质改善**（0.0000 → 0.0007）。
> 这条实验证据很重要：它排除了「训练不充分」这一解释，
> 确认问题出在置信度分支的机制本身（见 9.5 的根因分析）。

### 9.4 结论（这部分建议直接写进报告）

**1) 拿到了什么**

- **迭代版端到端检测流程**（滑窗 + 验证器/精修器融合）：
  测试集 **mAP@0.5 = 0.1435、召回 0.655**，比初版（0.0548）提升 **162%**；
  其中 PS 麻点 0.278、Pa 斑块 0.214 两类表现最好，召回分别达 0.80 / 0.78；
- 两个可靠的**缺陷区域判别模型**：验证器在测试集 GT 框上分类准确率 **97.00%**，
  精修器在此之上还能把候选框回归对齐（精修后平均 IoU 0.442）；
- **两套单阶段检测网络**（YOLOv3 / PP-YOLOE-s）的完整实现、训练、评估与可视化脚本，
  全部基于原生 Paddle，零新增依赖、不编译自定义 OP；
- **一套可复用的诊断工具链**（`tools/bottleneck*.py`、`sweep_refine.py`、`tune_fused.py`）——
  本轮迭代的方向就是靠它定位的，而不是靠试错。

**2) 单阶段网络没拿到什么，以及为什么（这是本次工作最有价值的部分之一）**

单阶段检测器的**框定位是好的，但「哪个框可信」学不出来**：

| 检查项 | 实测 | 说明 |
|---|---|---|
| GT 所属特征点上的框 IoU | **0.742 / 0.757** | 定位没问题 |
| 同一框的 obj 分数 | 0.056 | 在 307 个候选中排**第 135 位** |
| 结果 | mAP@0.5 = 0 | 被 top_k 截断淘汰 |
| 根因 | 正样本 2~3 个/图 vs 候选位置 10^4 量级（0.3%） | 置信度分支的类别极度不平衡 |

已尝试并**排除**的方案（都有实测数据支撑）：

| 尝试 | 结果 |
|---|---|
| 改置信度排序方式：obj×cls / 纯 cls / 纯 obj / obj×cls² | mAP 全部为 0 |
| 提高 top_k：100 → 300 → 1000 | 0.0002 → 0.0025 → 0.0025（触顶） |
| 用边缘能量（梯度幅值）当分数 | 0（该数据集背景本身纹理就重，边缘不是有效线索） |
| obj 分支偏置初始化 -4.5 / -2.0 / 0 | 无实质改善 |
| obj 损失改为正负样本各自归一化 | 无实质改善 |
| k-means 定制 anchor（IoU 0.567→0.692） | 定位略好，mAP 仍为 0 |
| 统一训练/验证输入尺度（原来随机缩放导致不一致） | 修正了一个真 bug，但 mAP 仍为 0 |
| 双阶段：检测器候选 + 验证器重打分 | 仍为 0，因为正确框排在 135 位、被候选数上限截断 |
| 按目标尺寸分档评估（`viz_det.py --figs size_ap`） | 大/中/小框的 mAP **全部为 0** —— 排除「小目标难检」这一解释 |
| **把训练跑满**（YOLOv3 40→120 epoch、PP-YOLOE-s 17→100 epoch） | 0.0000 → **0.0007**（YOLOv3）、0.0000（PP-YOLOE-s）—— 训练轮数翻三倍几乎没有变化，**排除「训练不充分」** |

**3) 根因分析**

单阶段检测器的置信度分支本质上是「在 10^4 量级的候选里做二分类、正样本只有 2~3 个」。
本数据集只有 1800 张图、平均每图 2.3 个框，正样本的绝对数量太少，
网络学不出「这个位置的框好不好」这个判断 —— 它能把框回归准（因为那是稠密监督），
但学不出可靠的置信度（那是极端不平衡的稀疏监督）。

这也解释了为什么**滑窗 + 验证器能工作**：它把同一件事（判断一块区域是不是缺陷）
变成了**稠密监督的分类任务**，每个训练样本都有明确标签，不存在正负样本悬殊的问题。

**4) 下一步改进建议**

1. 给置信度分支加**真正有效的重采样/损失**：Focal Loss 或 OHEM（只回传损失最大的
   若干负样本），而不是简单地调 pos_weight；
2. 换用**有预训练权重**的主干（本项目受限于 PaddleDetection 的 numpy<2 依赖无法引入，
   若后续能单独拿到 backbone 权重，对小数据集帮助会很大）；
3. 把验证器的定位也做起来：在滑窗基础上加一个**框回归头**（类似 RPN 的 refine），
   把 IoU 从目前的 0.507 提到 0.6+ 应能显著改善 mAP@0.5；
4. 数据层面：把 200×200 的原图做重叠裁块扩充训练样本数，
   可缓解小数据集下正样本过少的问题。

### 9.6 一键复现：完整流水线

最终交付的全部数字由 **一个脚本**产出，不是手工按顺序敲命令：

```bash
python src/det/tools/full_pipeline.py --device gpu
```

六个阶段（每阶段完成写一个 `.done` 标记到 `results/logs/_pipeline/`，
重跑自动跳过已完成的阶段，`--force` 可强制重跑）：

| 阶段 | 内容 | 本次实测耗时 |
|---|---|---|
| data | 数据交付自检（A 的 23 项） | 0s（很快） |
| detect | 训练 YOLOv3（120 epoch）+ PP-YOLOE-s（100 epoch） | 2417s + 5151s |
| region | 训练区域验证器（30 epoch）+ 精修器（40 epoch） | 162s + 1073s |
| eval | 两阶段基线 ×2 + 融合方案（验证集/测试集） | 258s + 257s + 910s + 928s |
| viz | 单阶段可视化 | 8s |
| report | 汇总结果表 + 文档一致性校验 | 2s + 2s |
| **合计** | | **186.2 分钟** |

本次完整重跑的训练结果：

| 模型 | epoch | 参数量 | 结果 | 耗时 |
|---|---|---|---|---|
| YOLOv3 | 120 | 6440.10 万 | 验证 mAP@0.5 = 0.0000 | 40.2 分钟 |
| PP-YOLOE-s | 100 | 2375.17 万 | 验证 mAP@0.5 = 0.0000 | 85.8 分钟 |
| 区域验证器 | 30 | 258.67 万 | 图块准确率 0.8816 | 2.6 分钟 |
| 区域精修器 | 40 | 259.08 万 | 验证准确率 0.7795 | 17.8 分钟 |

> 两个单阶段网络的验证 mAP 仍为 0（与 9.4 的根因分析一致：置信度分支学不出
> 判别力）。**最终交付的检测能力来自「滑窗 + 双模型融合」流程**，
> 单阶段网络作为对照基线保留。
>
> 12 个子任务中 11 项通过，唯一「失败」的是文档一致性校验 ——
> 它正是设计来发现「文档数字与产物不一致」的，本次重跑后数值有变化，
> 因此触发并促使我同步了文档（见 `tools/check_docs.py`）。

---

### 9.5 关于测试集

按项目铁律，测试集只在最终评估时使用。本任务中测试集在下列脚本中被读取过：
`eval_2stage.py --split test`（滑窗与检测器两种模式）、
以及验证器在 GT 框上的分类准确率统计。
训练阶段（`train.py` / `train_verifier.py`）**始终只加载 train 与 val**，
测试集未参与任何调参决策。

