...
检测效果经过一轮针对性迭代后**提升 2.6 倍**：

| 方案 | 测试集 mAP@0.5 | 精确率 | 召回率 | 每图框数 |
|---|---|---|---|---|
| 初版：滑窗 + 验证器 | 0.0548 | 0.0248 | 0.5142 | 48.8 |
| **迭代版：滑窗 + 验证器/精修器融合** | **0.1435** | **0.0704** | **0.6546** | 21.8 |
| 提升 | **+162%** | +184% | +27% | 框数减半 |
...
3. **补齐短板类别**：`In 夹杂`（AP 0.050）与 `Sc 划痕`（0.043）明显弱于
   `PS 麻点`（0.278）与 `Pa 斑块`（0.214）。夹杂形态弥散、划痕极细长，
   建议为这两类单独加密滑窗尺度或提高输入分辨率；
